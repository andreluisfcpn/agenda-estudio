                                                ...
                                                <div style={{ display: 'flex', gap: '4px', justifyContent: 'center', flexWrap: 'wrap' }}>
                                                    ...
                                                    {c.status === 'ACTIVE' && (
                                                        <button className="btn btn-ghost btn-sm" title="Pausar" style={{ fontSize: '0.8125rem', padding: '4px 8px', borderRadius: '8px' }}
                                                            onClick={() => {
                                                                showConfirm({ title: '⏸️ Pausar Contrato', message: `Pausar "${c.name}"? Bookings futuros serão cancelados.`, onConfirm: async () => { try { const r = await contractsApi.pause(c.id, { reason: 'Pausa administrativa' }); showToast(r.message); loadData(); } catch (e: unknown) { showToast(getErrorMessage(e) || 'Erro'); } } });
                                                            }}>⏸️</button>
                                                    )}
                                                    {(c.status as string) === 'PAUSED' && (
                                                        <button className="btn btn-sm" title="Retomar" style={{ background: 'rgba(16,185,129,0.12)', color: '#10b981', border: 'none', fontSize: '0.8125rem', padding: '4px 8px', borderRadius: '8px', fontWeight: 600, cursor: 'pointer' }}
                                                            onClick={() => {
                                                                showConfirm({ title: '▶️ Retomar Contrato', message: `Retomar "${c.name}"? Vigência será estendida.`, onConfirm: async () => { try { const r = await contractsApi.resume(c.id); showToast(r.message); loadData(); } catch (e: unknown) { showToast(getErrorMessage(e) || 'Erro'); } } });
                                                            }}>▶️</button>
                                                    )}
                                                </div>
                                            </td>
