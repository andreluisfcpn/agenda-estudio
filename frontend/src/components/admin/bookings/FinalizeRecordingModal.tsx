...
import { Flag, Radio, Timer, Globe, Link2, NotebookPen, MessageCircle } from 'lucide-react';
    ...
    return (
        <BottomSheetModal isOpen onClose={onClose} hideHeader size="md" className="admin-sheet" title="Finalizar gravação">
            <div style={{ padding: '24px 28px 28px' }}>
                <h2 style={{ fontSize: '1.125rem', fontWeight: 800, margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 8 }}><Flag size={18} style={{ color: 'var(--success)' }} aria-hidden="true" /> Finalizar gravação</h2>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '12px 14px', borderRadius: 10, marginBottom: 14, background: isLive ? 'rgba(239,68,68,0.06)' : 'var(--bg-elevated)', border: `1px solid ${isLive ? 'rgba(239,68,68,0.3)' : 'var(--border-default)'}` }}>
                    ...
                    <div>
                        <div style={{ fontSize: '0.8125rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}><Radio size={14} style={{ color: 'var(--danger)' }} aria-hidden="true" /> Foi transmissão ao vivo?</div>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                    ...
                    <button onClick={save} disabled={saving} className="btn-admin-go">
                        {saving ? 'Salvando…' : <><Flag size={15} aria-hidden="true" /> Finalizar</>}
                    </button>
                </div>
            </div>
        </BottomSheetModal>
