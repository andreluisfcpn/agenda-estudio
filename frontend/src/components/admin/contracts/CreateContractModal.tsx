...
import {
    ...
    Building2, Mic, Star, Wallet, Zap, CreditCard, TicketPercent, AlertTriangle,
    CheckCircle2, Ban,
} from 'lucide-react';
            ...
            <BottomSheetModal isOpen onClose={onClose} hideHeader size="lg" className="admin-sheet" title="Novo Contrato">
                    ...
                    <div style={{ padding: '28px 32px 0' }}>
                        <h2 style={{ fontSize: '1.25rem', fontWeight: 800, margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <span style={{ width: 36, height: 36, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--accent-gradient-go)', fontSize: '1rem' }}><FileText size={16} aria-hidden="true" /></span>
                            Novo Contrato
                        </h2>
                    ...
                    <div style={{ padding: '20px 32px 28px' }}>
                        ...
                        {createSuccess && <div className="admin-alert admin-alert--success">{createSuccess}</div>}

                        {/* --- SECTION 1: Cliente & Projeto --- */}
                        <div style={{ marginBottom: '20px' }}>
                            <SectionHeader num={1} label="Cliente & Projeto" color="var(--success)" />
                        ...
                        <div style={{ marginBottom: '20px' }}>
                            <SectionHeader num={2} label="Configuração do Contrato" />
                            ...
                            <div style={{ marginBottom: '14px' }}>
                                ...
                                <div className="admin-grid-2" style={{ gap: '8px' }}>
                                    {[
                                        { key: 'FIXO', icon: Pin, label: 'Fixo', desc: 'Recorrente: dia/hora fixos toda semana' },
                                        { key: 'FLEX', icon: RefreshCw, label: 'Flex', desc: 'Créditos: agende quando quiser' },
                                    ].map(t => (
                                        <button key={t.key} onClick={() => setCreateForm({ ...createForm, type: t.key as any })}
                                            ...
                                            <div style={{ fontSize: '0.875rem', fontWeight: 700, color: createForm.type === t.key ? (t.key === 'FIXO' ? 'var(--accent-text)' : '#10b981') : 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '6px' }}><t.icon size={14} aria-hidden="true" /> {t.label}</div>
                            ...
                            <div style={{ marginBottom: '14px' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '6px' }}>
                                    {[
                                        { key: 'COMERCIAL', icon: Building2, label: 'Comercial', color: '#10b981' },
                                        { key: 'AUDIENCIA', icon: Mic, label: 'Audiência', color: '#2dd4bf' },
                                        { key: 'SABADO', icon: Star, label: 'Sábado', color: '#fbbf24' },
                                    ].map(t => (
                                        <button key={t.key} onClick={() => setCreateForm({ ...createForm, tier: t.key as any })}
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'center' }}><t.icon size={16} aria-hidden="true" /></div>
                            ...
                            {createForm.type === 'FIXO' && (
                                <div style={{ padding: '14px', borderRadius: '10px', background: 'rgba(99,102,241,0.04)', border: '1px solid rgba(99,102,241,0.1)', marginBottom: '14px' }}>
                                    <div style={{ fontSize: '0.625rem', fontWeight: 700, color: 'var(--accent-text)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '10px' }}><RefreshCw size={11} style={{ verticalAlign: '-2px', marginRight: 4 }} aria-hidden="true" />Configuração Recorrente</div>
                            ...
                            {createForm.type === 'FLEX' && (
                                <div style={{ padding: '12px 14px', borderRadius: '10px', background: 'rgba(16,185,129,0.04)', border: '1px solid rgba(16,185,129,0.1)', marginBottom: '14px', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                                    <div style={{ fontWeight: 700, color: '#10b981', marginBottom: '6px', fontSize: '0.6875rem' }}><RefreshCw size={12} style={{ verticalAlign: '-2px', marginRight: 4 }} aria-hidden="true" />Regras Flex</div>
                            ...
                            <div style={{ marginBottom: '14px' }}>
                                ...
                                <div className="admin-grid-2" style={{ gap: '8px' }}>
                                    {[
                                        { key: 'MONTHLY' as const, icon: CalendarDays, label: 'Mensal', desc: `${createForm.durationMonths || 3} parcelas` },
                                    ...
                                    {([
                                        ...
                                        return (
                                            <button key={m.key} onClick={() => setPaymentMethod(m.key)}
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'center' }}><m.icon size={16} aria-hidden="true" /></div>
                        ...
                        {addons.length > 0 && (
                            <div style={{ marginBottom: '20px' }}>
                                <SectionHeader num={3} label="Serviços por Gravação" color="#2dd4bf" />
                        ...
                        {tierPrice && (
                            <div style={{ marginBottom: '20px' }}>
                                <SectionHeader num={4} label="Estimativa de Preço" color="var(--warning)" />
